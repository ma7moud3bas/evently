        ...
        <aside className="h-full bg-gray-800 pt-5 flex flex-col justify-between max-md:hidden" {...props}>
            <div className="px-4">
                {links.map((link, index) => (
                    <Fragment key={index}>
                        <Link href={link.link} onClick={() => handleExpand(index)} className="flex w-full gap-x-8 items-center justify-between py-2 px-2.5 cursor-pointer transition-all hover:bg-gray-600">
